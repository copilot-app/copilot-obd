...
void configureUartPort(UartConfiguration configuration) {
    ...
    ESP_LOGI(__func__, "UART port number %d configured", portNumber);
}

void startUartRxTransmission(RxTaskParameters* parameters) {
    ESP_LOGI(__func__, "Starting UART transmission on port number %d", parameters->portNumber);

    xTaskCreate(rxTask, "uart_rx_task", 1024 * 2, (void*)parameters, configMAX_PRIORITIES, NULL);

    ESP_LOGI(__func__, "UART transmission on port number %d started", parameters->portNumber);
}
